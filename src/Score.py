...
def Kruskal(arcs, nombre_villes):
    ...
    while len(arbre_minimal) != nombre_villes - 1 :
        (depart, arrivee, longueur) = arcs[index]
        index = index + 1
...
# liste des arcs au format (départ, arrivée, longueur)
    ...
    arcs = [("Los Angeles", "San Franciso", 3),("Los Angeles", "Las Vegas", 2),
    ...
    arcs.sort(key=lambda x: x[2]) # on trie les liaisons par longueur
    nombre_villes = 36

    print(Kruskal(arcs, nombre_villes))